...
- miner_burn: **0.7274701758287847**
- registration cost: 0.2 TAO (62.54600000000001 USD), open=True
- tempo: 360.0  |  max_uids: 256  |  active: 11  |  free: 0
- subnet age: 996.9 days  |  registered at block 1970929
- weights_version: 1022  |  mechanisms: 1

## Income (miner side)

- **competitive_miner_usd_day: [UNKNOWN]** (uid [UNKNOWN]) <- the only figure quotable as achievable
- median_miner_usd_day: 1054.0316829361207
- top_miner_usd_day: 1533.5522462323868 (uid 240, owner=True, validator_permitted=True) <- NOT achievable if owner or permitted

## Incentive structure (display only - never scored)

- earners: 2  |  gini: 0.22746971037934505  |  top1_share: 0.7274697103793452  |  top10_share: 1.0
- owner_incentive_share: 0.727469710379345 (independent check on miner_burn; disagreement 0.0)
...
- status: **ok** 
- README: 6168 bytes, sha 4367b7b34b3e13ab
- latest release: (none) 
- last commit: 2026-09-24T16:34:09Z
- scoring-related commit: Ground native semantic scoring in clip evidence 2026-09-24T16:34:09Z
...
The current task contract requires **English** responses. Existing miners that
only accept Spanish tasks must update, and the English judge prompt requires its
own calibration before semantic scores can be qualified.

The opt-in [ActivityNet edits profile](docs/activitynet-edits-v1.md) reorders,
repeats and mixes annotated video blocks and adds speech, text and sounds. It
uses the same MP4 transport and scoring, with a separate ranking history.
Its implementation checks do not establish resistance to caption retrieval;
generic descriptions of added content remain a known scoring limitation.

The separate [native model challenge implementation](docs/native-challenges.md)
adds local 10→20 paired evaluation, `0.8 × factual F1 + 0.2 × speed`, immutable
model manifests and signed evaluator/follower weight aggregation. Its simulation
uses synthetic miners and receipts; production MP4 5.2 and burn remain unchanged.
...
_(truncated at 6000 of 6168 chars - read the full file at https://github.com/witnessvision/witness_subnet)_
