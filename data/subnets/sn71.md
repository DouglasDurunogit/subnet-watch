...
- subnet age: 543.5 days  |  registered at block 5048438
- weights_version: 0  |  mechanisms: 1

## Income (miner side)

- **competitive_miner_usd_day: [UNKNOWN]** (uid [UNKNOWN]) <- the only figure quotable as achievable
- median_miner_usd_day: 3256.9260330103248
- top_miner_usd_day: 3256.9260330103248 (uid 0, owner=True, validator_permitted=True) <- NOT achievable if owner or permitted
...
- status: **ok** 
- README: 12146 bytes, sha 3232e3bea944578f
- latest release: v2-cutover-legacy-final-certfix-20260720 2026-07-20T19:10:50Z
- last commit: 2026-08-30T07:15:47Z
- scoring-related commit: Document miner SOURCE_ADD inputs (#153) 2026-08-30T00:13:57Z
...
After the model proposes a patch, the system checks that the patch only touches files the model actually read during that loop and only within the allowed edit paths. If the patch passes those checks, it is applied to the extracted runtime source
```

_(truncated at 6000 of 12146 chars - read the full file at https://github.com/leadpoet/leadpoet)_
