...
- subnet age: 544.9 days  |  registered at block 5048438
- weights_version: 0  |  mechanisms: 1

## Income (miner side)

- **competitive_miner_usd_day: [UNKNOWN]** (uid [UNKNOWN]) <- the only figure quotable as achievable
- median_miner_usd_day: 3293.8771196833527
- top_miner_usd_day: 3293.8771196833527 (uid 0, owner=True, validator_permitted=True) <- NOT achievable if owner or permitted
...
After the model proposes a patch, the system checks that the patch only touches files the model actually read during that loop and only within the allowed edit paths. If the patch passes those checks, it i
```

_(truncated at 6000 of 12187 chars - read the full file at https://github.com/leadpoet/leadpoet)_
